...
func (c *eventServiceClient) StreamTestExecutionEvents(ctx context.Context, in *StreamTestExecutionEventsRequest, opts ...grpc.CallOption) (EventService_StreamTestExecutionEventsClient, error) {
	stream, err := c.cc.NewStream(ctx, &EventService_ServiceDesc.Streams[0], EventService_StreamTestExecutionEvents_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &eventServiceStreamTestExecutionEventsClient{stream}
...
func _EventService_StreamTestExecutionEvents_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(EventServiceServer).StreamTestExecutionEvents(m, &eventServiceStreamTestExecutionEventsServer{stream})
}
